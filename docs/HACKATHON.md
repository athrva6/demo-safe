...
| OCR | Amazon Textract | Extracts text and bounding boxes from screenshots |
...
The team learns a first serverless deployment, Cognito JWT authentication, Textract OCR, S3 privacy controls, DynamoDB TTL expiry, and API Gateway routing.

## Execution

`Upload screenshot → Textract suggestions → human review → flattened PNG → expiring share → revoke`
...
2. Upload it and run the Textract scan.
